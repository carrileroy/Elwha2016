...
#Now make a plot for Shannon's H (H.est): The normal ggboxplots are weird because the medians and means 
...
data_summary <- function(fungal, H.est, region){
  ...
  data_sum<-ddply(fungal, region, .fun=summary_func,
                  H.est)
  data_sum <- rename(data_sum, c("mean" = H.est))
  return(data_sum)
}
...
plotF <- p<- ggplot(fun2, aes(x=region, y=H.est, fill=region)) + 
...
ggsave(file="DivF.pdf", plotF, width=16.4, height=12.4, units="cm", dpi=800)
ggsave(plotF, file="DivF.eps", width=16.4, height=12.4, units="cm", dpi=800, device="eps")
ggsave(plotF, file="DivF.jpg", width=16.4, height=12.4, units="cm", dpi=800, device="jpg")
#
