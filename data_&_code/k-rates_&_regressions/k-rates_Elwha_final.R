...
#
# # For 2016 Elwha decomp paper: k-rate pANOVAs; and regressions with k and water quality/substrate variables
# Datasets = kAll.csv
...
# Regressions between k and initial litter chemistry values: use dataset kAll.csv 
kAll$region = factor(kAll$region,
                     levels=unique(kAll$region))
#
...
#
slr2 = lmp(elevation~k, data=kAll)
summary(slr2)
#
#Coefficients:
#  Estimate Iter Pr(Prob)  
...
slr51 = lmp(SimpsonM~k, data=kAll)
summary(slr51)
#Coefficients:
#  Estimate Iter Pr(Prob)
#k    3.048  659    0.132
