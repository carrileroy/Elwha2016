...
# Add significant Community vectors to plot
...
plot(bray.ord.EvenM, add = TRUE, cex=1.5)
plot(bray.ord.Div, add = TRUE, cex=1.5)
plot(bray.ord.SimpsonM, add = TRUE, cex=1.5)

# Add significant Physical vectors to plot
colvec <- c("orange2", "gold", "green1", "blue1", "purple1")
shpvec <- c(16, 15, 16, 15, 16, 15)
plot(bray.ord, type = "n", choices=c(1,2), cex.lab=1.5, cex.axis=1.3)
with(second, points(bray.ord, display = "sites", cex=1.5, col = colvec[region], pch = shpvec[region], bg = colvec[region]))
...
write.csv(nmds_coordinates, 'BMI_Elwha_points.csv')
...
#*************************SMC code for NMS*********************
# Dissimilarity index, where 0 = same composition, 1 = maximally dissimilar.
bray = vegdist(main, "bray") 
gower = vegdist(main, "gower")
par(mfrow = c(1, 2))
hist(bray, xlim = range(0.0, 1.0))
hist(gower, xlim = range(0.0, 1.0))
par(mfrow=c(1,1))

# Determine the best number of axes to use in an ordination.
...
# metaMDS function definitions and defaults:
# k = number of dimensions, default is 2.
# trymax = Maximum number of random starts in search of stable solution.
# distance default is "bray" Bray-Curtis dissimilarity.
# autotransform = if TRUE (default) applies possible data transformation of community data.
#  If data values are larger than common abundance class scales, the function performs 
#  Wisconsin double standardization. If the values are very large, the function also 
#  performs sqrt transformation. However, the limits are completely arbitrary 
#  (? data maximum 50 triggers sqrt and >9 triggers wisconsin). For full control,
#  set autotransform = FALSE. Standardize and transform data independently. 
# noshare = If TRUE (default), extended dissimilarities are used when there are 
#  no shared species between some sites. If FALSE, they are never used.
# wasscores = If TRUE (default), calculate species scores as weighted averages.
# center = If TRUE (default), moves the origin to the average of the axes.
# pc = If TRUE (default), Principle Component rotation, rotate the configuration so 
#  the variance of points is maximized on first dimension (alternately see function MDSrotate). 
# halfchange = If TRUE (default), scales axes to half-change units. If FALSE, the 
#  ordination dissimilarities are scaled to the same range as the input dissimilarities.

# Set a random number "seed" so always get the same NMS ordination output.
set.seed(1431)
m.mds <- metaMDS(main, distance = "bray", k=3, trymax = 100)
#m.mds <- metaMDS(mainlog, distance = "bray", k=3, trymax = 100, 
#        autotransform=FALSE, noshare=FALSE, wasscores=TRUE)
m.mds
m.mds$species # ordination coordinates of taxa
m.mds$points  # ordination coordinates of samples (same as data.scores)
stressplot(m.mds)
ordiplot(m.mds, display = "site", type = "text", choices=c(1,2)) #Axes 1 & 2
ordiplot(m.mds, display = "site", type = "text", choices=c(1,3)) #Axes 1 & 3
